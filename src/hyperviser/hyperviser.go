...
var serverAddrs = map[AddrPair]bool{
	...
	AddrPair{"54.218.109.234", defaultPort}:  true,
	AddrPair{"54.218.19.111", defaultPort}:   true,
	AddrPair{"54.67.120.138", defaultPort}:   true,
	AddrPair{"54.215.199.227", defaultPort}:  true,
	AddrPair{"204.236.252.178", defaultPort}: true,
	AddrPair{"54.91.15.118", defaultPort}:    true,
	AddrPair{"13.58.96.173", defaultPort}:    true,
	AddrPair{"52.15.171.117", defaultPort}:   true,
	AddrPair{"52.14.79.149", defaultPort}:    true,
}
...
func (hv *Hyperviser) StartLeader(testType TestType, leaderLog, testLog string) error {
	...
	for batchNum, batchSize := range tests[testType].phases {
		...
		timeout := time.Minute*time.Duration(info.targetNumChs)
...
func makePort(ip, pass, logDir string, port int) (*Hyperviser, error) {
	...
	hv.servListener, err = net.Listen("tcp", fmt.Sprintf("0.0.0.0:%d", port))
	if err != nil {
		return nil, err
	}
...
var tests = map[TestType]testConfig{
	LookupPerf: testConfig{
		...
		phases:  []int{60, 90, 120, 150, 180, 200},
		f:       lookupPerf,
